...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">{{ __('lables.user-edit') }}</h1>
            </div><!-- /.col -->
             <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">{{__('lables.user-edit')}}</li> 
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="card main-card ">
                    <!-- /.card-header -->
                    <!-- form start -->
                    <form id="circle-form" class="p-4" method="POST" action="{{route('user_update')}}">
                    @csrf
                    <div class="card-body p-0">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="{{ __('placeholders.enter-name') }}" value="{{ $userData->name}}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" id="email" placeholder="{{ __('placeholders.enter-email') }}" value="{{ $userData->email}}" readonly>
                            @error('email')
                                <span class="alert text-danger pl-0 pb-0 fs-14 app_slug_err mb-0 error_msg" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="mobile_no" class="form-control @error('mobile_no') is-invalid @enderror only_number" id="mobile_no" placeholder="Enter mobile number" value="{{ $userData->mobile_no}}" maxlength="10">
                            @error('mobile_no')
                                <span class="alert text-danger pl-0 pb-0 fs-14 app_slug_err mb-0 error_msg" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="user_name" class="form-control @error('name') is-invalid @enderror alphabets_numbers_underscore" id="user_name" placeholder="Enter username" value="{{ $userData->user_name}}">
                            @error('user_name')
                                <span class="alert text-danger pl-0 pb-0 fs-14 app_slug_err mb-0 error_msg" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <select name="roles"  id="roles" class="js-states form-control @error('master_ce_id') is-invalid @enderror"  data-placeholder="select role" data-dropdown-css-class="select2-green" style="width: 100%;" >
                            <option value="">Select role</option>
                                @foreach ($roles as $role)
                                        <option value="{{$role->id}}" {{ ($role->id == $userData->roles[0]->id) ? 'selected' : '' }}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                            @error('roles')
                                <span class="alert text-danger pl-0 pb-0 fs-14 app_slug_err mb-0 error_msg" role="alert">
